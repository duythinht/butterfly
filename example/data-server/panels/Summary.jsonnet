...
local targets = [
  ...
  g.target.Prometheus.new(
    'sum(op_duration_count{op="Query"})',
    legend='Total Queries Executed',
    instant=true,
    refId='B'
  ),
  g.target.Prometheus.new(
    'sum(increase(cdc_op_counter[$__range]))',
    legend='CDC Ops Overtime',
    refId='C'
  ),
  g.target.Prometheus.new(
    'sum(increase(engine_completed{}[$__range]))',
    legend='Ops Completed Overtime',
    refId='D'
  ),
  g.target.Prometheus.new(
    'sum(cdc_stream_total_op_count)',
    legend='Total Stream Ops',
    refId='E'
  ),
  g.target.Prometheus.new(
    'sum(cdc_stream_total_msg_count{})',
    legend='Total Stream Message',
    refId='F'
  ),
  g.target.Prometheus.new(
    'count(cdc_op_per_table_counter{op="c"}) by (op)',
    legend='Total CDC Tables',
    refId='G'
  ),
  g.target.Prometheus.new(
    'engine_running{op="engine.query"}',
